class Observation < ActiveRecord::Base
  ...
  scope :recent, lambda {|number| {order: 'obs_datetime DESC,date_created DESC', limit: number}}
  scope :old, lambda {|number| {order: 'obs_datetime ASC,date_created ASC', limit: number}}
  ...
  def answer_string(tags=[])
    ...
    if (["HTC Test 1 Time frame", "HTC Test 2 Time frame"].include?(self.concept.concept_names.last.name) rescue false)
      return (coded_name.sub(/\.0$/, "") + (!self.value_complex.blank? ? ("[" + self.value_complex + "]") : "")) unless coded_name.blank?
    end
  ...
  def self.new_accession_number
    last_accn_number = self.maximum(:accession_number).to_s rescue "00" #the rescue is for the initial accession number start up
    ...
    chk_dgt = ClientIdentifier.calculate_checkdigit(new_accn_number_with_no_chk_dgt)
  ...
  def to_piped_s(tags=[])
    formatted_name = self.concept_name.typed(tags, self.concept_name.concept_id).name rescue nil
    formatted_name ||= self.concept.concept_names.typed(tags, self.concept.concept_id).first.name || self.concept.fullname rescue nil
    formatted_name ||= self.concept.concept_names.first.name rescue 'Unknown concept name'
    "#{formatted_name}|  #{self.answer_string(tags)}"
  end
end
