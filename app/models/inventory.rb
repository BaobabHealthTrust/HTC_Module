class Inventory < ActiveRecord::Base
  ...
  def self.remaining_sum(date, lot_number, type)
    ...
    damages_type = InventoryType.where(name: "Losses").first.id
    ...
    damages =  Inventory.find_by_sql(["SELECT SUM(iv.value_numeric) AS total FROM inventory iv
                                          INNER JOIN  inventory iv2 ON iv.lot_no = iv2.lot_no AND iv2.inventory_type = #{del_type}
                                          WHERE iv.inventory_type = ? AND iv.voided = 0 AND iv.lot_no = ?
                                          AND DATE(iv.encounter_date) <= ? AND DATE(iv2.date_of_expiry) > ?",
