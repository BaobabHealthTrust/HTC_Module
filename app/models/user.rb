class User < ActiveRecord::Base
  ...
  def remaining_stock_by_type(kit_type = nil, date = Date.today, users = [], type = "closing")
    result = 0
    eq = "<="
    eq = "<" if (type == "opening")
    ...
    CouncillorInventory.find_by_sql(
          ["SELECT ci.id, ci.value_numeric, ci.inventory_type FROM councillor_inventory ci
            ...
            AND iv.kit_type = ? AND DATE(iv.date_of_expiry) > ? AND DATE(ci.encounter_date) #{eq} ?
          WHERE ci.councillor_id IN (?) GROUP BY ci.id, ci.councillor_id",
           Kit.find_by_name(kit_type).id, date.to_date, date.to_date, users]).each do |iv|
